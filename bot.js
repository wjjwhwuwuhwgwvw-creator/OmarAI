    ...
    for (const file of pluginFiles) {
        try {
            ...
            if (plugin.default && plugin.default.commands && plugin.default.handler) {
                ...
                console.log(`✅ Command Plugin تحمّل: ${plugin.default.name || file}`);
            }
        } catch (error) {
...
async function humanDelay(phone = null) {
    ...
    // تأخير ثابت 1 ثانية فقط بدون إضافات عشوائية
    if (baseDelay > 0) {
...
async function simulateTyping(sock, remoteJid, textLength = 50) {
    try {
        ...
        const charTime = Math.min(textLength * 20, 3000);
        const totalTime = baseTime + charTime + Math.random() * 1500;
...
async function connectToWhatsApp() {
    ...
    const sock = makeWASocket({
        ...
        transactionOpts: { maxCommitRetries: 10, delayBetweenTriesMs: 2000 },
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        try {
            ...
            if (!shouldProcessMessage(isGroup, isAdmin)) {
                const modeText = BOT_MODE === 'groups' ? 'الجروبات فقط' : 'الخاص فقط';
                console.log(`🔇 تم تجاهل الرسالة - البوت يعمل في ${modeText}`);
                return;
            }
